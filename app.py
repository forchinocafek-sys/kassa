...
# Ініціалізація дати ДО малювання інтерфейсу
...
st.markdown("""
<style>
    /* Змушуємо колонки триматися в один рядок на всіх екранах */
...
st.caption("🌐 Хмарна синхронізація | Stable 1.3 (Dual Tab & Mobile Fix)")
...
with tab1:
    ...
    if not st.session_state.get("edit_ok", False):
        ...
        with col_b2:
            ...
            # Компактна таблиця фактів
            def cash_row_live(label, multiplier):
                c1, c2, c3 = st.columns([1, 2, 2])
                with c1: 
                    st.markdown(f"<div style='margin-top: 8px; font-weight: bold;'>{label}</div>", unsafe_allow_html=True)
                with c2: 
                    qty = get_int(st.text_input(f"q{label}", label_visibility="collapsed", key=f"qty_{label}_{selected_date}"))
                with c3: 
                    st.markdown(f"<div style='margin-top: 8px;'>= {qty * multiplier}</div>", unsafe_allow_html=True)
