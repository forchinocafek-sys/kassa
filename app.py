...
EXPENSE_CHOICES = [f"{group} ➔ {sub}" for group, subs in EXPENSE_TREE.items() for sub in subs]
...
def get_previous_advances(date_str):
    try:
        ...
        if isinstance(res, list) and len(res) > 0:
            ...
            if last_date:
                adv = requests.get(f"{SUPABASE_URL}/rest/v1/advances?date=eq.{last_date}", headers=headers).json()
                if isinstance(adv, list): return [{"Співробітник": i.get('employee', ''), "Сума": get_int(i.get('amount', 0)), "Примітка": ""} for i in adv]
    except: pass
    return []
    ...
    try:
        ...
        if isinstance(res, list) and len(res) > 0:
            ...
            if last_date:
                ...
                if draft: return get_int(draft[0].get('payload', {}).get('cash', {}).get('coins', 0))
    except: pass
    return 0
...
def prefetch_week_window(center_date_obj):
    ...
    sd = (center_date_obj - timedelta(days=3)).strftime('%Y-%m-%d')
    ed = (center_date_obj + timedelta(days=3)).strftime('%Y-%m-%d')
    try:
        res = requests.get(f"{SUPABASE_URL}/rest/v1/drafts?date=gte.{sd}&date=lte.{ed}", headers=headers).json()
        if isinstance(res, list):
            for r in res: st.session_state["drafts_cache"][r.get('date')] = r.get('payload', {})
    except: pass
...
def upload_receipts_to_supabase(date_str, receipts_list):
    ...
    for r in receipts_list:
        file_path = f"{date_str}/{r['id']}_{r['name'].replace(' ', '_').replace('/', '-')}"
        try:
            res = requests.post(f"{SUPABASE_URL}/storage/v1/object/receipts/{file_path}", headers=upload_headers, data=r['bytes'])
            if res.status_code not in [200, 201]: errors.append(f"{r['name']}: {res.text}")
        except Exception as e: errors.append(f"{r['name']}: {e}")
    if errors:
        st.error("❌ Деякі чеки не завантажилися:")
...
def load_draft_or_init(date_str):
    ...
    if "drafts_cache" in st.session_state and date_str in st.session_state["drafts_cache"]:
        ...
        clean_exp = []
        for e in exp_loaded:
            cat_val = e.get("Категорія", f"{e.get('Група')} ➔ {e.get('Підкатегорія')}" if "Група" in e else EXPENSE_CHOICES[0])
        ...
        for k in [20, 50, 100, 200, 500, 1000]: st.session_state[f"qty_{k}_{date_str}"] = str(cash_data.get(str(k), 0)) if cash_data.get(str(k), 0) else ""
        return
...
manifest = {
    "name": "Cafe Forchino", "short_name": "Forchino", "theme_color": "#FAF0E6", "background_color": "#FAF0E6",
    "display": "standalone", "orientation": "portrait", "icons": [{"src": ICON_URL, "sizes": "512x512", "type": "image/png"}]
}
components.html(f"<script>const doc=window.parent.document; let m=doc.createElement('link'); m.rel='manifest'; m.href='data:application/manifest+json;base64,{base64.b64encode(json.dumps(manifest).encode()).decode()}'; doc.head.appendChild(m);</script>", height=0, width=0)
...
with st.popover("🚀 Версія: 2.9.0 (Access & Security)"):
    st.markdown("**Оновлення v2.9.0:** Впроваджено персональні паролі, рівні доступу (обмеження редагування минулих дат) та прихований журнал аудиту дій персоналу.")
...
# Визначення чи можна редагувати обраний день
can_edit = False
...
if user_role == "admin":
    ...
    if selected_date_obj >= yesterday: can_edit = True
...
if st.session_state["active_tab"] == "Касса":
    ...
    with col_t1:
        ...
        edited_inc_df = st.data_editor(prepare_df(st.session_state["inc_data"], ["Категорія", "Сума", "Примітка"]),
            column_config={"Категорія": st.column_config.SelectboxColumn(options=INCOME_CATEGORIES, required=True), "Сума": st.column_config.NumberColumn(min_value=0, step=1)},
            num_rows="dynamic", use_container_width=True, key=f"inc_{selected_date}", disabled=not can_edit)
    ...
    with col_t2:
        ...
        edited_exp_df = st.data_editor(prepare_df(st.session_state["exp_data"], ["Категорія", "Сума", "Примітка"]),
            column_config={"Категорія": st.column_config.SelectboxColumn(options=EXPENSE_CHOICES, required=True), "Сума": st.column_config.NumberColumn(min_value=0, step=1)},
            num_rows="dynamic", use_container_width=True, key=f"exp_{selected_date}", disabled=not can_edit)
    ...
    with col_b1:
        st.subheader("💸Аванси:")
        edited_adv_df = st.data_editor(prepare_df(st.session_state["adv_data"], ["Співробітник", "Сума", "Примітка"]), 
            num_rows="dynamic", use_container_width=True, key=f"adv_{selected_date}", disabled=not can_edit)
    ...
    with col_b2:
        st.subheader("💰Факт")
        m_coins = get_int(st.text_input("Монети (загальна сума):", placeholder="0", key=f"coins_{selected_date}", disabled=not can_edit))
        ...
        q_20, v_20 = cash_row("20", 20); q_50, v_50 = cash_row("50", 50)
        q_100, v_100 = cash_row("100", 100); q_200, v_200 = cash_row("200", 200)
        q_500, v_500 = cash_row("500", 500); q_1000, v_1000 = cash_row("1000", 1000)
    ...
    if can_edit:
        if st.button("🚀 ЗБЕРЕГТИ ФІНАЛЬНИЙ ЗВІТ", type="primary", use_container_width=True):
            with st.spinner("Стерилізація та відправка звіту..."):
                ...
                st.session_state["drafts_cache"][selected_date] = payload; st.cache_data.clear() 
                ...
                if res_shift.status_code in [200, 201]:
                    inc_rows = [{"date": selected_date, "type": "income", "description": f"{cat} | {note}" if note else cat, "amount": str(amt)} for _, r in edited_inc_df.iterrows() for amt, cat, note in [(get_int(r.get("Сума", 0)), str(r.get("Категорія", "")).strip(), str(r.get("Примітка", "")).strip())] if amt or cat]
                    exp_rows = [{"date": selected_date, "type": "expense", "description": f"{cat} | {note}" if note else cat, "amount": str(amt)} for _, r in edited_exp_df.iterrows() for amt, cat, note in [(get_int(r.get("Сума", 0)), str(r.get("Категорія", "")).strip(), str(r.get("Примітка", "")).strip())] if amt or cat]
                    adv_rows = [{"date": selected_date, "employee": emp, "amount": str(amt), "note": str(r.get("Примітка", "")).strip() if pd.notna(r.get("Примітка", "")) and str(r.get("Примітка", "")).lower()!='nan' else ""} for _, r in edited_adv_df.iterrows() for amt, emp in [(get_int(r.get("Сума", 0)), str(r.get("Співробітник", "")).strip())] if amt or emp]
                ...
                else: st.error(f"❌ Помилка: {res_shift.text}")
...
# ==========================================
    ...
    shift_res = requests.get(f"{SUPABASE_URL}/rest/v1/shifts?date=eq.{selected_date}", headers=headers).json()
    if isinstance(shift_res, list) and len(shift_res) > 0:
        shift = shift_res[0]
        st.markdown(f"<h3 style='margin-bottom: 0;'>🌅 Залишок на початок: <span style='color: #0066cc;'>{get_int(shift.get('start_balance'))} грн</span></h3>", unsafe_allow_html=True); st.divider()
        ...
        with ac1:
            ...
            total_inc = 0
            for item in inc_res or []:
                amt = get_int(item.get('amount')); total_inc += amt
        ...
        with ac2:
            ...
            total_exp = 0
            for item in exp_res or []:
                amt = get_int(item.get('amount')); total_exp += amt
                parts = item.get('description', 'Без опису').split(' | ', 1)
                st.markdown(f"• {parts[0]}: {amt} грн{' <i>— '+parts[1]+'</i>' if len(parts)>1 else ''}", unsafe_allow_html=True)
            st.markdown(f"<p style='font-weight: bold; color: #c62828;'>Загалом: {total_exp} грн</p>", unsafe_allow_html=True)
                
        st.divider(); st.markdown(f"<h3 style='margin-bottom: 0;'>🌇 Залишок на кінець: <span style='color: #0066cc;'>{get_int(shift.get('calculated_end'))} грн</span></h3>", unsafe_allow_html=True); st.divider()
        
        st.subheader("🟠 Аванси")
        adv_res = requests.get(f"{SUPABASE_URL}/rest/v1/advances?date=eq.{selected_date}", headers=headers).json()
        total_adv = 0
        for item in adv_res or []:
            amt = get_int(item.get('amount')); total_adv += amt
    ...
    with c_btn:
        if can_edit:
            with st.popover("📷 Додати чеки"):
                ufs = st.file_uploader("Виберіть файли", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key=f"upl_{selected_date}")
                if st.button("➕ Відправити на сервер", use_container_width=True) and ufs:
                    with st.spinner("Завантаження..."):
                        receipts_to_upload = [{"id": str(uuid.uuid4()), "name": uf.name, "bytes": (img:=Image.open(uf).convert("RGB") if Image.open(uf).mode in ("RGBA", "P") else Image.open(uf), img.thumbnail((1024, 1024)), buf:=io.BytesIO(), img.save(buf, format="JPEG", quality=70), buf.getvalue())[4]} for uf in ufs]
                        if upload_receipts_to_supabase(selected_date, receipts_to_upload):
                            ...
                            st.success("Успішно!"); time.sleep(1); st.rerun() 
                        ...
                        if can_edit:
                            with st.popover("🗑️ Видалити", use_container_width=True):
                                ...
                                if st.button("Так", key=f"del_{file_name}", type="primary"):
                                    if requests.delete(f"{SUPABASE_URL}/storage/v1/object/receipts/{selected_date}/{file_name}", headers=headers).status_code in [200, 204]: 
...
# ==========================================
    ...
    if st.button("🚀 Згенерувати матрицю PnL", type="primary", use_container_width=True):
        with st.spinner("Збір даних..."):
            ...
            order_full = ["Касса на начало дня", "🟢 НАДХОДЖЕННЯ"] + INCOME_CATEGORIES + ["🔴 ВИТРАТИ"] + expense_groups_list + ["Інші (старі ручні записи)", "🔴 ВСЬОГО ВИТРАТ", "Касса на конец дня"]
            report_data = {cat: {str(d): {"sum": 0, "notes": [], "set": False} for d in range(1, num_days + 1)} for cat in order_full}

            for s in (requests.get(f"{SUPABASE_URL}/rest/v1/shifts?date=gte.{start_d}&date=lt.{end_d}", headers=headers).json() if isinstance(requests.get(f"{SUPABASE_URL}/rest/v1/shifts?date=gte.{start_d}&date=lt.{end_d}", headers=headers).json(), list) else []):
                day = str(int(s['date'].split('-')[2]))
                report_data["Касса на начало дня"][day]["sum"] = get_int(s.get('start_balance', 0)); report_data["Касса на начало дня"][day]["set"] = True
                report_data["Касса на конец дня"][day]["sum"] = get_int(s.get('actual_end', 0)); report_data["Касса на конец дня"][day]["set"] = True

            for t in (requests.get(f"{SUPABASE_URL}/rest/v1/transactions?date=gte.{start_d}&date=lt.{end_d}", headers=headers).json() if isinstance(requests.get(f"{SUPABASE_URL}/rest/v1/transactions?date=gte.{start_d}&date=lt.{end_d}", headers=headers).json(), list) else []):
                day, amt, parts = str(int(t['date'].split('-')[2])), get_int(t.get('amount', 0)), t.get('description', '').strip().split(' | ', 1)
                left_part, note = parts[0].strip(), parts[1].strip() if len(parts) > 1 else ""

                if t.get('type') == 'income':
                    target_inc = left_part if left_part in INCOME_CATEGORIES else "Разное"
                    report_data[target_inc][day]["sum"] += amt
                    if note_text := (note if note else (left_part if target_inc == "Разное" else "")): report_data[target_inc][day]["notes"].append(f"{amt} {note_text}")
                else:
                    group_name = left_part.split(' ➔ ')[0].strip() if ' ➔ ' in left_part else (left_part.split(' >> ')[0].strip() if ' >> ' in left_part else left_part)
                    target_cat = group_name if group_name in report_data else "Інші (старі ручні записи)"
                    report_data[target_cat][day]["sum"] += amt
                    sub_cat = left_part.split(' ➔ ')[1].strip() if ' ➔ ' in left_part else (left_part.split(' >> ')[1].strip() if ' >> ' in left_part else "")
                    
                    if full_note_parts := [p for p in [sub_cat, note] if p]: report_data[target_cat][day]["notes"].append(f"{amt} {' - '.join(full_note_parts)}")
                    elif target_cat == "Інші (старі ручні записи)" and group_name: report_data[target_cat][day]["notes"].append(f"{amt} {group_name}")

            for d in range(1, num_days + 1):
                day_total = sum(report_data[cat][str(d)]["sum"] for cat in expense_groups_list + ["Інші (старі ручні записи)"])
                report_data["🔴 ВСЬОГО ВИТРАТ"][str(d)]["sum"] = day_total
                if day_total > 0: report_data["🔴 ВСЬОГО ВИТРАТ"][str(d)]["set"] = True

            df_rows = []
            for r in order_full:
                row_dict, row_total = {"Стаття": r}, 0 
                for d in range(1, num_days + 1):
                    cell = report_data[r][str(d)]
                    if r in ["🟢 НАДХОДЖЕННЯ", "🔴 ВИТРАТИ"]: row_dict[str(d)] = ""
                    elif r in ["Касса на начало дня", "Касса на конец дня", "🔴 ВСЬОГО ВИТРАТ"]: row_dict[str(d)], row_total = str(cell["sum"]) if cell["set"] else "", row_total + cell["sum"]
                    else:
                        if cell["sum"] == 0 and not cell["notes"]: row_dict[str(d)] = ""
                        else:
                            row_total += cell["sum"]
                            row_dict[str(d)] = f"{cell['sum']} ({', '.join([n for n in cell['notes'] if n])})" if [n for n in cell["notes"] if n] else str(cell["sum"])
                row_dict["Всього"] = "" if r in ["🟢 НАДХОДЖЕННЯ", "🔴 ВИТРАТИ", "Касса на начало дня", "Касса на конец дня"] else (str(row_total) if row_total > 0 else "")
                df_rows.append(row_dict)
                
            def style_pnl(row):
                styles = [''] * len(row); styles[-1] = 'background-color: #f3f4f6; font-weight: bold; border-left: 2px solid #d1d5db;'
            ...
            st.dataframe(pd.DataFrame(df_rows).style.apply(style_pnl, axis=1), use_container_width=True, hide_index=True)
...
with fc1:
    ...
    if "Касса" in st.session_state["allowed_tabs"] and st.session_state["active_tab"] != "Касса":
        if st.button("🧮", key="nav_kas"): st.session_state["active_tab"] = "Касса"; st.rerun()
with fc2:
    if "Архів" in st.session_state["allowed_tabs"] and st.session_state["active_tab"] != "Архів":
        if st.button("🗃️", key="nav_arch"): st.session_state["active_tab"] = "Архів"; st.rerun()
with fc3:
    if "Сличительная" in st.session_state["allowed_tabs"] and st.session_state["active_tab"] != "Сличительная":
        if st.button("📊", key="nav_pnl"): st.session_state["active_tab"] = "Сличительная"; st.rerun()
with fc4:
    with st.popover("📅"):
